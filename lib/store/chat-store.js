        ...
        _separateReasoningPart: (conversationId, nodeId) => { // New action
          set(state => {
            ...
            if (node && node.role === 'assistant' && node.reasoning && node.reasoning.length > 0) {
              // Add a newline if the current reasoning text doesn't already end with one,
              // to ensure parts are separated for display.
              if (!node.reasoning.endsWith('\n')) {
                node.reasoning += '\n';
              }
              state.conversations[conversationId].updatedAt = new Date().toISOString();
            }
          ...
          try {
            if (response.stream) {
              ...
              const onChunk = (chunkOrEvent) => {
                const isReasoningModel = response.provider === 'openai' && response.isReasoningModel;
                if (typeof chunkOrEvent === 'string') {
                  ...
                  switch (chunkOrEvent.type) {
                    ...
                    case 'openai_reasoning_summary_delta':
                      ...
                      break;
                    case 'openai_reasoning_summary_part_ended':
                      if (isReasoningModel) {
                        get()._separateReasoningPart(conversationId, targetAssistantNodeId);
                      }
                      break;
                    case 'openai_reasoning_all_processing_done':
                      if (isReasoningModel) {
              ...
              if (response.provider === 'openai' && response.reasoningSummary) {
                const isReasoningModel = response.isReasoningModel; // from OpenAIService
                if (isReasoningModel) {
                  get()._setReasoningInProgress(conversationId, targetAssistantNodeId, true); // Simulate start
                  get()._finalizeReasoningSummary(conversationId, targetAssistantNodeId, response.reasoningSummary);
                }
              }
