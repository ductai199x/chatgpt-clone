  ...
  immer(
    persist(
        ...
        */
          ...
          try {
            ...
            set(state => {
              ...
              if (parentUserNode) {
                ...
                if (!parentUserNode.childrenMessageIds.includes(newUserMessageNodeId)) {
                  parentUserNode.childrenMessageIds = [...parentUserNode.childrenMessageIds, newUserMessageNodeId];
                }
                // Set as the active next message (switch to the edited version)
                parentUserNode.nextMessageId = newUserMessageNodeId;
              } else {
            ...
            set(state => {
              ...
              if (parentNode) {
                ...
                if (!parentNode.childrenMessageIds.includes(newAssistantNodeId)) {
                  parentNode.childrenMessageIds = [...parentNode.childrenMessageIds, newAssistantNodeId];
                }
        ...
        deleteMessageBranch: (conversationId, messageNodeIdToDelete) => {
          set(state => {
            ...
            if (parentNode.childrenMessageIds) {
              ...
              if (childIndex > -1) {
                parentNode.childrenMessageIds = parentNode.childrenMessageIds.filter((_, i) => i !== childIndex);
              }
            }
            ...
            if (activeChain.length > 0) {
              ...
              if (prevNode) {
                ...
                if (!prevNode.childrenMessageIds.includes(newNode.id)) {
                  prevNode.childrenMessageIds = [...prevNode.childrenMessageIds, newNode.id];
                }
              }
            } else {
            ...
            if (node) {
              ...
              if (typeof node.content === 'string') {
                ...
                if (textPartIndex !== -1) {
                  ...
                  node.content = [...node.content, { type: 'text', text: chunk }];
                }
              }
              state.conversations[conversationId].updatedAt = new Date().toISOString();
            }
        ...
        _addNewReasoningStep: (conversationId, nodeId) => {
          set(state => {
            ...
            if (node && node.role === 'assistant') {
              ...
              node.reasoning = [...node.reasoning, newStep];
            }
          });
        },
        ...
        _addToolUseStep: (conversationId, nodeId, tool) => {
          set(state => {
            ...
            if (node && node.role === 'assistant') {
              ...
              node.reasoning = [...node.reasoning, toolStep];
              if (node.reasoning.length === 0) {
                node.reasoningStartTime = Date.now();
                node.reasoningDurationMs = null;
              }
        ...
        _createArtifactNode: (conversationId, originatingMessageNodeId, metadata = {}) => {
          ...
          set(state => {
            ...
            originatingNode.artifactsCreated = [...originatingNode.artifactsCreated, generatedId]; // Link to message
            conv.updatedAt = newNode.createdAt;
          });
        ...
        _updateArtifactNode: (conversationId, originatingMessageNodeId, prevNodeId, metadata = {}) => {
          ...
          set(state => {
            ...
            originatingNode.artifactsCreated = [...originatingNode.artifactsCreated, generatedId];
      ...
      }),
      {
        ...
        version: 2, // Increment to version 2 for reasoning array migration
        migrate: async (persistedState, version) => {
          console.log(`Attempting migration from version ${version} to 2...`);
