        ...
        editUserMessage: async (conversationId, messageNodeIdToEdit, newContent, referencedNodeIds = []) => {
          ...
          try {
            ...
            set(state => {
              ...
              if (parentUserNode) {
                // Get the parent node from the Immer draft state
                const draftParentNode = currentConv.message_nodes[parentUserNode.id];
                if (draftParentNode) {
                  // Add to parent's children if not already there (similar to regenerateResponse)
                  if (!draftParentNode.childrenMessageIds.includes(newUserMessageNodeId)) {
                    draftParentNode.childrenMessageIds.push(newUserMessageNodeId);
                  }
                  // Set as the active next message (switch to the edited version)
                  draftParentNode.nextMessageId = newUserMessageNodeId;
                }
              } else {
