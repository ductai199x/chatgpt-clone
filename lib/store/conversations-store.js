...
export const useConversationsStore = create(immer((set, get) => ({
   ...
   * Includes artifact context and continuation instructions if needed.
   * @param {string} conversationId
  ...
  _prepareApiContext: (
    ...
    // 1. Format Artifact Context
    let artifactContextString = '';
    try {
      const artifacts = useArtifactsStore.getState().getArtifactsForConversation(conversationId);
      const artifactEntries = Object.values(artifacts || {});

      if (artifactEntries.length > 0) {
        const artifactTags = artifactEntries.map(artifact => {
          const statusAttr = !artifact.isComplete ? ' status="incomplete"' : '';
          const langAttr = artifact.metadata?.language ? ` language="${artifact.metadata.language}"` : '';
          const filenameAttr = artifact.metadata?.filename ? ` filename="${artifact.metadata.filename}"` : '';
          const typeAttr = ` type="${artifact.type || 'unknown'}"`;
          const idAttr = ` id="${artifact.id}"`;
          ...
          const safeContent = (artifact.content || '').replace(/]]>/g, ']]]]><![CDATA[>');
      ...
      if (messagesForApi[lastMsgIndex].role === 'user') {
        ...
        if (typeof userContent === 'string') {
          finalUserContent += userContent;
            ...
            if (textPartIndex !== -1) {
                userContent[textPartIndex].text = finalUserContent + userContent[textPartIndex].text;
                messagesForApi[lastMsgIndex].content = userContent; // Assign the modified array
            } else {
