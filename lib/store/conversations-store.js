...
export const useConversationsStore = create(immer((set, get) => ({
  ...
  /**
   * Prepares the message history and context for an API call.
   * Includes artifact context and continuation instructions if needed. (using the *latest* version of each artifact)
   * @param {string} conversationId
   * @param {object} conversation - The conversation object (use a fresh copy from get() if needed).
   * @param {boolean} isContinuation - Whether this is a continuation request.
   * @param {string | object[]} userContent - The raw content of the user's message.
   * @param {string | null} [formatUntilMessageId=null] - For regeneration, format history up to this message ID.
   * @param {string | null} [formatUntilVersionId=null] - Corresponding version ID for formatUntilMessageId.
  ...
  _prepareApiContext: (
    ...
    isContinuation,
    userContent,
    formatUntilMessageId = null,
    formatUntilVersionId = null
  ) => {
    const settings = useSettingsStore.getState();
    const { currentProvider, systemPrompt } = settings;

    // 1. Format Artifact Context using the LATEST version of each artifact
    let artifactContextString = '';
    try {
      const artifactsMap = useArtifactsStore.getState().getArtifactsForConversation(conversationId);
      const artifactContainers = Object.values(artifactsMap || {});

      if (artifactContainers.length > 0) {
        const artifactTags = artifactContainers.map(artifactContainer => {
          if (!artifactContainer || !artifactContainer.versions || artifactContainer.versions.length === 0) {
            console.warn(`[_prepareApiContext] Artifact container ${artifactContainer?.id} has no versions.`);
            return null; // Skip this artifact if it has no versions
          }

          // --- Get the LATEST version ---
          const latestVersion = artifactContainer.versions[artifactContainer.versions.length - 1];
          if (!latestVersion) {
            console.warn(`[_prepareApiContext] Could not get latest version for artifact ${artifactContainer.id}.`);
            return null; // Skip if latest version is somehow invalid
          }
          // --- Use properties from the LATEST version ---
          const statusAttr = !latestVersion.isComplete ? ' status="incomplete"' : '';
          const langAttr = latestVersion.metadata?.language ? ` language="${latestVersion.metadata.language}"` : '';
          const filenameAttr = latestVersion.metadata?.filename ? ` filename="${latestVersion.metadata.filename}"` : '';
          const typeAttr = ` type="${latestVersion.type || 'unknown'}"`;
          const idAttr = ` id="${latestVersion.id}"`;

          // Basic XML escaping for content within CDATA (though CDATA handles most cases)
          // Primarily needed if the content itself could contain ']]>'
          const safeContent = (latestVersion.content || '').replace(/]]>/g, ']]]]><![CDATA[>');

          return `  <artifact${idAttr}${typeAttr}${langAttr}${filenameAttr}${statusAttr}><![CDATA[${safeContent}]]></artifact>`;
        }).join('\n');

        if (artifactTags) { // Only add context if there were valid tags
          artifactContextString = `<artifacts_context>\n${artifactTags}\n</artifacts_context>\n\n`;
        }
      }
    } catch (error) {
      console.error("Error formatting artifact context:", error);
      // Proceed without artifact context if formatting fails
    }

    // 2. Format Base Messages
    const messagesForApi = formatMessagesForProvider(
      conversation,
      currentProvider,
      systemPrompt,
      formatUntilMessageId,
      formatUntilVersionId
    );

    // 3. Inject Context & Instructions into the last user message
    if (messagesForApi.length > 0) {
      const lastMsgIndex = messagesForApi.length - 1;
      // Ensure the last message is the one we intend to modify (should be the user's turn)
      if (messagesForApi[lastMsgIndex].role === 'user') {
        let finalUserContent = artifactContextString; // Start with artifact context

        if (isContinuation) {
          finalUserContent += `Please continue exactly where you left off, completing the previous artifact.\n\n`;
        }

        // Append the actual user message content
        // Handle potential multimodal content structure
        if (typeof userContent === 'string') {
          finalUserContent += userContent;
        } else if (Array.isArray(userContent)) {
            // Find the text part and prepend, keep image parts as is
            const textPartIndex = userContent.findIndex(part => part.type === 'text');
            if (textPartIndex !== -1) {
              // Create a mutable copy if needed, or ensure formatMessagesForProvider returns mutable objects
              const modifiedContent = [...userContent]; // Create shallow copy
              modifiedContent[textPartIndex] = { ...modifiedContent[textPartIndex], text: finalUserContent + modifiedContent[textPartIndex].text };
              messagesForApi[lastMsgIndex].content = modifiedContent;
            } else {
              // If no text part, prepend as a new text part (might depend on provider requirements)
              messagesForApi[lastMsgIndex].content = [{ type: 'text', text: finalUserContent }, ...userContent];
            }
        } else {
             // Fallback if userContent is unexpected type, treat as string
             finalUserContent += String(userContent);
             messagesForApi[lastMsgIndex].content = finalUserContent;
        }
         // If content was originally a string, update it directly
         if (typeof userContent === 'string') {
            messagesForApi[lastMsgIndex].content = finalUserContent;
         }

      } else {
        console.warn("[_prepareApiContext] Last message in formatted history is not 'user'. Cannot inject context easily.");
        // Potential fallback: Add a new system message with the context? Less ideal.
        // messagesForApi.push({ role: 'system', content: artifactContextString + (isContinuation ? 'Continue instruction...' : '') });
      }
    } else if (artifactContextString) {
        // If messagesForApi is empty but we have context (e.g., very first message?),
        // this case needs careful consideration based on API requirements.
        // Maybe add a system message?
        console.warn("[_prepareApiContext] Message history is empty, but artifact context exists.");
    }
   ...
   * Handles marking previous messages as complete after successful continuation.
   * @private
   */
  _handleApiResponse: async (
    ...
    initialArtifactIdForParser = null, // ID if continuing an artifact
    previousAssistantVersionInfo = null // { messageId, versionId } of the message being continued | null
  ) => {
    let parser = null;
    ...
    try {
      if (response.stream) {
        ...
        // 1. Pass initialArtifactIdForParser to the parser
        ...
        const onDone = () => {
          ...
          // Determine final state of the *current* message version
          ...
          // 2. Check if a continuation was successfully completed
          if (previousAssistantVersionInfo && !isNowIncomplete) {
            console.log(`[onDone] Continuation successful, marking previous message ${previousAssistantVersionInfo.messageId} / ${previousAssistantVersionInfo.versionId} as complete.`);
            // Mark the *original* incomplete message as complete
            get()._finalizeMessageVersionState(
              conversationId,
              previousAssistantVersionInfo.messageId,
              previousAssistantVersionInfo.versionId,
              false, // No longer incomplete
              null   // Clear incomplete artifact ID
            );
          }
        ...
        const onError = (error) => {
          ...
          set(draft => {
            ...
            if (version) {
              ...
              version.isIncomplete = false; // Ensure marked as complete on error
              version.incompleteArtifactId = null;
            }
            // Also ensure the *previous* message isn't left incomplete if continuation failed
            if (previousAssistantVersionInfo) {
              const prevVersion = draft.conversations[conversationId]?.messages?.[previousAssistantVersionInfo.messageId]?.versions?.find(v => v.id === previousAssistantVersionInfo.versionId);
              if (prevVersion && prevVersion.isIncomplete) {
                  console.warn(`[onError] Marking previous incomplete message ${previousAssistantVersionInfo.messageId} / ${previousAssistantVersionInfo.versionId} as complete due to continuation error.`);
                  prevVersion.isIncomplete = false;
                  prevVersion.incompleteArtifactId = null;
              }
            }
        ...
        // 2. Check if a continuation was successfully completed (even though non-streaming)
        if (previousAssistantVersionInfo && !isNowIncomplete) {
          console.log(`[Non-Stream] Continuation successful, marking previous message ${previousAssistantVersionInfo.messageId} / ${previousAssistantVersionInfo.versionId} as complete.`);
          // Mark the *original* incomplete message as complete
          get()._finalizeMessageVersionState(
            conversationId,
            previousAssistantVersionInfo.messageId,
      ...
      set(draft => {
        ...
        if (previousAssistantVersionInfo) {
          const prevVersion = draft.conversations[conversationId]?.messages?.[previousAssistantVersionInfo.messageId]?.versions?.find(v => v.id === previousAssistantVersionInfo.versionId);
          if (prevVersion && prevVersion.isIncomplete) {
              console.warn(`[_handleApiResponse Error] Marking previous incomplete message ${previousAssistantVersionInfo.messageId} / ${previousAssistantVersionInfo.versionId} as complete due to continuation error.`);
              prevVersion.isIncomplete = false;
              prevVersion.incompleteArtifactId = null;
          }
        }
  ...
  sendMessage: async (conversationId, content, images = []) => {
    ...
    let isContinuation = false; // Flag to indicate if this is a continuation
    ...
    try {
      ...
      // 1. Add User Message
      // Use the original content, even if it's "continue"
      const userMessageContent = images?.length > 0 ? [{ type: 'text', text: content }, ...images.map(img => ({ type: 'image_url', image_url: { url: img.url } }))] : content;
      userMessageNode = createMessage({ role: 'user', content: userMessageContent });
      ...
      // 2. Prepare API Call using the new helper function
      const { createApiService } = await import('@/lib/api/api-service');
      const apiService = createApiService(currentProvider, apiKey);

      // Call the helper to get formatted messages including artifact context and continuation prompt
      const messagesForApi = get()._prepareApiContext(
        conversationId,
        conversationForApi, // Pass the latest conversation state
        isContinuation,     // Pass the flag
        userMessageContent  // Pass the original user content
        // formatUntil... args are null for sendMessage
      );
      console.log(`[sendMessage] Formatted messages for API:`, messagesForApi);
  ...
  regenerateResponse: async (conversationId, messageIdToRegenerate) => {
    ...
    const precedingUserVersion = precedingUserNode?.versions?.find(v => v.id === precedingVersionId);

    if (!precedingUserNode || !precedingUserVersion) {
      console.error("[regenerateResponse] Could not find the preceding user message version.");
      set(draft => { draft.error = "Internal error: Could not find context for regeneration."; });
      return;
    }
    const triggeringUserContent = precedingUserVersion.content; // Get the content that triggered the original response

    // --- Check for Continuation Scenario ---
    let isContinuation = false;
    let initialArtifactIdForParser = null;
    let previousAssistantVersionInfo = null; // Refers to the message *before* the 'continue' user message

    // Check if the user message *before* the assistant message being regenerated was 'continue'
    if (precedingUserNode.role === 'user' && typeof triggeringUserContent === 'string' && triggeringUserContent.trim().toLowerCase() === 'continue') {
        ...
        const prevAssistantVersion = prevAssistantNode?.versions?.find(v => v.id === prevAssistantVersionId);
        if (prevAssistantVersion?.isIncomplete && prevAssistantVersion.incompleteArtifactId) {
            isContinuation = true; // Set the flag
            initialArtifactIdForParser = prevAssistantVersion.incompleteArtifactId;
    ...
    try {
      // 1. Create placeholder version, set active, update predecessor pointer
      set((draft) => {
        const conv = draft.conversations[conversationId];
        ...
        if (precedingMessageId && precedingVersionId) {
          const predNode = conv.messages[precedingMessageId];
          const predVersion = predNode?.versions?.find(v => v.id === precedingVersionId);
          if (predVersion) {
            predVersion.nextMessageId = messageIdToRegenerate; // Ensure forward link exists
            predVersion.nextMessageVersionId = tempNewVersionId; // Point to the new version
          }
        }
      ...
      // --- Get conversation state *after* adding the new version ---
      const conversationForApi = get().conversations[conversationId];
      if (!conversationForApi) throw new Error("Conversation disappeared after adding new version.");

      // 2. Prepare API Call using the helper function
      const { createApiService } = await import('@/lib/api/api-service');
      const apiService = createApiService(currentProvider, apiKey);
      ...
      const messagesForApi = get()._prepareApiContext(
        ...
        conversationForApi,       // Pass the latest conversation state
        isContinuation,           // Pass the flag
        triggeringUserContent,    // Pass the content of the user message that triggered the original response
        precedingMessageId,       // Format history *up to* the preceding user message
        precedingVersionId
      );
