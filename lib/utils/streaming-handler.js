      ...
      for (const line of lines) {
        if (line.startsWith('data: ')) {
          ...
          try {
            ...
            if (contentDelta) {
              // Emit message_started event only once
              if (!messageStarted) {
                onChunk({ type: 'message_started' });
                messageStarted = true;
              }
              ...
              onChunk({ type: 'message_delta', content: contentDelta });
            }
          } catch (err) {
            console.error('Error parsing Google AI SSE data:', err);
          }
        }
      }
    }

    // Emit message_done event if we started a message
    if (messageStarted) {
      onChunk({ type: 'message_done' });
    }
