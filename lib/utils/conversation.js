...
export function formatMessagesForProvider(conversation, provider, systemPrompt = null, endMessageId = null, endVersionId = null) {
  ...
  if (trimmedUserPrompt) {
    ...
    // Include empty tags if user prompt is empty but artifact instructions exist
    combinedSystemContent += `<system_prompt></system_prompt>\n\n`;
  }
  // Append artifact instructions regardless of user prompt presence
  combinedSystemContent += `<artifact_instruction>\n${ARTIFACT_INSTRUCTIONS.trim()}\n</artifact_instruction>`;
  // --- End System Prompt Construction ---
  console.log(`[formatMessagesForProvider] Combined system content: ${combinedSystemContent}`);

  // Prepend system prompt if provided
  const messagesToFormat = combinedSystemContent
    ? [{ role: 'system', content: combinedSystemContent, id: 'system', versionId: 'system' }, ...activeChain]
    : activeChain;
  console.log(`[formatMessagesForProvider] Messages to format:`, messagesToFormat);
  ...
  switch (provider) {
    case 'openai':
      return messagesToFormat.map(message => {
        ...
        // Ensure content is not an empty array if it only contained invalid parts
        if (Array.isArray(content) && content.length === 0) {
          content = ''; // Send empty string instead of empty array for OpenAI
        }
        return { role, content };
